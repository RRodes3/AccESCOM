...
// AccessLog helper
async function createAccessLog({ kind, action, userId, guestId, qrId, guardId }) {
...
router.post('/issue', auth, requireRole(['USER', 'ADMIN']), async (req, res) => {
  try {
    ...
    await createAccessLog({ kind, action: 'ISSUE', userId: req.user.id, qrId: pass.id });
...
router.post('/validate', auth, requireRole(['GUARD', 'ADMIN']), async (req, res) => {
  try {
    ...
    if (pass.userId && pass.user) {
      ...
      await prisma.user.update({ where: { id: u.id }, data: { accessState: newState } });
      await createAccessLog({
        kind: pass.kind,
      ...
      const ownerAllowed = buildOwner({ ...pass, user: { ...u, accessState: newState } });

      const successReason = pass.kind === 'EXIT' ? 'Salida permitida' : 'Acceso permitido';
    ...
    if (pass.guestId && pass.guest) {
      ...
      await prisma.guestVisit.update({ where: { id: g.id }, data: { state: newState } });
...
router.post('/reset-state', auth, async (req, res) => {
  try {
    await prisma.user.update({ where: { id: req.user.id }, data: { accessState: 'OUTSIDE' } });
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
...
router.post('/scan', auth, requireRole(['GUARD', 'ADMIN']), async (req, res) => {
  ...
  try {
    ...
    if (pass && pass.guestId && result === 'ALLOWED') {
      ...
      if (pass.guest) {
        const newState = pass.kind === 'ENTRY' ? 'INSIDE' : 'COMPLETED';
        await prisma.guestVisit.update({ where: { id: pass.guest.id }, data: { state: newState } });
      }
    }
    ...
    if (
      ...
      setImmediate(() => {
        const u = pass.user;
        const recipient = u.contactEmail || u.email;
        console.log('📧 Notificación acceso permitido (scan) a:', recipient);
        sendAccessNotificationEmail({
          to: recipient,
          name: buildFullName(u),
          type: accessType,
          date: new Date(),
          locationName: 'ESCOM',
        }).catch(err => console.error('Email acceso async (scan):', err));
      });
    }
  } catch (e) {
