 ...
 * @param {string} actionOnConflict - 'overwrite' | 'skip' (default: 'overwrite')
 * @returns {Promise<Object>} - Resultado con estadísticas de la importación
 */
async function importCSV(csvPath, actionOnConflict = 'overwrite') {
  const users = [];
  const errors = [];

  return new Promise((resolve, reject) => {
    if (!fs.existsSync(csvPath)) {
      return reject(new Error(`Archivo no encontrado: ${csvPath}`));
    }

    fs.createReadStream(csvPath)
      .pipe(csvParser())
      .on('data', (row) => {
        // Normalizar nombres de columnas (quitar espacios, acentos, etc.)
        const normalizedRow = {};
      ...
      .on('end', async () => {
        ...
        for (let i = 0; i < users.length; i++) {
          const row = users[i];
          const lineNumber = i + 2; // +2 porque la fila 1 es encabezado
          ...
          try {
            ...
            if (existingUser && actionOnConflict === 'skip') {
